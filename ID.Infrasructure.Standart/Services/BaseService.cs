...
{
    public abstract class BaseService<TDbContext> : IUnitOfWork, IDisposable, IBaseService<TDbContext> where TDbContext : DbContext
    {
        ...
        public TDbContext DbContext
        {
            get
            {
                if (_dbContext == null)
                {
                    _dbContext = GeneralContext.GetService<TDbContext>();
        ...
        public Serilog.ILogger Logger
        {
            get
            {
                ...
                Serilog.ILogger logger = GeneralContext.GetService<Serilog.ILogger>();
        ...
        public virtual TService GetChildService<TService>() where TService : IBaseService<TDbContext>
        {
        ...
        public virtual TRepository GetChildRepository<TRepository>() where TRepository : IBaseRepository<TDbContext>
        {
        ...
        public TService GetRootService<TService>() where TService : BaseService<TDbContext>
        {
            _serviceScope = GeneralContext.CreateServiceScope();
            var service = GeneralContext.GetService<TService>();
            service.DbContext = _serviceScope.ServiceProvider.GetService<TDbContext>();
        ...
        {
            return _changesCounter += this.DbContext.SaveChanges();
        }
        ...
        {
            return _changesCounter += await this.DbContext.SaveChangesAsync(cancellationToken);
        }
        ...
        public virtual async Task<int> CommitTransactionAsync(CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                changesCount = this.GetChangesCounter();
                if (changesCount == 0 && IsAutoSaveChangesEnabled)
