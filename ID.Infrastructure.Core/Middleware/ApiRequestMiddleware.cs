...
namespace ID.Infrastructure.Middleware
{
    public class ApiRequestMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            ...
            var token = context.GetRequestHeader(HttpRequestHeader.Authorization.ToString());
            ...
            var dataString = context.GetRequestHeader(HttpRequestXHeader.Data.ToString());
            if (!string.IsNullOrEmpty(dataString))
            {
                var data = JsonConvert.DeserializeObject<Dictionary<string, string>>(context.GetRequestHeader(HttpRequestXHeader.Data.ToString()));
                if (data != null)
                {
