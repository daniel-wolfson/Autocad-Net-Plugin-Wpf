...
namespace ID.Infrastructure
{
    ...
    public class GeneralHttpClient : IDisposable
    {
        ...
        public GeneralHttpClient(ApiServiceNames namedhttpClient, HttpClient httpClient)
        {
            if (httpClient == null)
            {
                //var configuration = Plugin.GetService<IConfiguration>();
                //var endpoint = configuration.GetSection("AppConfig:Endpoints:CalcApi").Value;
                var appConfig = Plugin.GetService<IAppConfig>();
                var host = appConfig.Endpoints.FirstOrDefault(x => x.Key == namedhttpClient.ToString()).Value;
                httpClient = new HttpClient { BaseAddress = new Uri($"{host}/api/{namedhttpClient}/") };
                httpClient.DefaultRequestHeaders.Add("User-Agent", appConfig.Domain);
                httpClient.DefaultRequestHeaders.Add("X-Named-Client", namedhttpClient.ToString());
                httpClient.DefaultRequestHeaders.Add(HttpRequestHeader.Authorization.ToString(), "token");
        ...
        public async Task<TResult> ExecuteAsync<TResult>(HttpMethod httpMethod, string path, object data, string contentType = null)
        {
            ...
            try
            {
                ...
                using (var httpContent = CreareHttpContent(path, data, contentType))
                {
                    ...
                    httpResponse = _httpClient.SendAsync(httpRequest).GetAwaiter().GetResult();
                }
            }
            catch (Exception ex)
            {
        ...
        {
            HttpContent content;
            contentType = path.Contains("compress=true") ? CustomMediaTypeNames.CrpmZip : contentType;
            ...
            switch (contentType)
            {
                ...
                case CustomMediaTypeNames.CrpmZip:
                ...
                default:
                    ...
                    content = new StringContent(JsonConvert.SerializeObject(data), Encoding.UTF8, CustomMediaTypeNames.Json);
                    break;
            }
        ...
        {
            HttpContent content;
            RequestUrl = _httpClient.BaseAddress.ToString() + path;
            ...
            switch (contentType)
            {
                ...
                case "CustomMediaTypeNames.CrpmZip":
                default:
                    contentType = CustomMediaTypeNames.Json;
            ...
            var httpRequestMessage = new HttpRequestMessage
            {
                ...
                Headers = { { HttpRequestHeader.ContentType.ToString(), contentType } },
                Content = content
            };
