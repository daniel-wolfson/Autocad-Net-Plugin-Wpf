...
namespace Intellidesk.AcadNet.Common.Internal
{
    ...
    public static class Modify // todo: need to add overloads for multiple ids
    {
        ...
        public static ObjectId Offset(this ObjectId curveId, double distance, Point3d side)
        {
            ...
            return (curve1.GetDistToPoint(side) < curve2.GetDistToPoint(side)
                ? curve1
                : curve2).AddToCurrentSpace();
        }
        ...
        public static ObjectId Mirror(this ObjectId entityId, Line axis, bool copy = true)
        {
            ...
            return mirror.AddToCurrentSpace();
        }
        ...
        public static void EraseGroup(this ObjectId groupId)
        {
            DbHelper
                ...
                .Cast<ObjectId>()
                .XForEach(entity => entity.Erase());

            groupId.XOpenForWrite(group => group.Erase());
        }
        ...
        public static void Ungroup(ObjectId groupId)
        {
            ...
            groupDictId.XOpenForWrite<DBDictionary>(groupDict => groupDict.Remove(groupId));
            groupId.Erase();
        }
        ...
        {
            var groupDict = DbHelper.GetDatabase(entityIds).GroupDictionaryId.XOpenForRead<DBDictionary>();
            var count = 0;
            foreach (var entry in groupDict)
            {
                var group = entry.Value.XOpenForRead<Group>();
                if (entityIds.Any(entityId => group.Has(entityId.XOpenForRead<Entity>())))
                {
                    Ungroup(entry.Value);
                    count++;
                }
        ...
        {
            entityId.Database.BlockTableId.XOpenForWrite<BlockTable>(blockTable =>
            {
                blockTable[BlockTableRecord.ModelSpace].XOpenForWrite<BlockTableRecord>(blockTableRecord =>
                {
                    blockTableRecord.DrawOrderTableId.XOpenForWrite<DrawOrderTable>(drawOrderTable =>
                    {
                        switch (operation)
                        {
        ...
        public static void Draworder(this IEnumerable<ObjectId> entityIds, DraworderOperation operation)
        {
            DbHelper
                ...
                .BlockTableId
                .XOpenForWrite<BlockTable>(blockTable =>
                {
                    blockTable[BlockTableRecord.ModelSpace].XOpenForWrite<BlockTableRecord>(blockTableRecord =>
                    {
                        blockTableRecord.DrawOrderTableId.XOpenForWrite<DrawOrderTable>(drawOrderTable =>
        ...
        public static ObjectId TextStyle(string fontFamily, double textHeight, double italicAngle = 0, double xScale = 1, bool vertical = false, string bigFont = "", string textStyleName = Consts.TextStyleName)
        {
            ...
            result.XOpenForWrite<TextStyleTableRecord>(tstr =>
            {
        ...
        {
            entityId.XOpenForWrite<Entity>(entity =>
            {
