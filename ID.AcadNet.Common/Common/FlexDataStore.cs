...
namespace Intellidesk.AcadNet.Common
{
    ...
    public class FlexDataStore
    {
        ...
        {
            var dictionary = DictionaryId.XOpenForRead<DBDictionary>();
            if (dictionary.Contains(key))
            {
                var record = dictionary.GetAt(key).XOpenForRead<Xrecord>();
        ...
        public FlexDataStore SetValue(string key, string value)
        {
            ...
            {
                var dictionary = trans.GetObject(DictionaryId, OpenMode.ForWrite) as DBDictionary;
                if (dictionary.Contains(key))
                {
    ...
    public static class FlexDataStoreExtensions
    {
        ...
        public static FlexDataStore FlexDataStore(this Database db)
        {
            using (var trans = db.TransactionManager.StartTransaction())
            {
                ...
                if (!namedObjectsDict.Contains(DwgGlobalStoreName))
                {
                    namedObjectsDict.UpgradeOpen();
                    var dwgGlobalStore = new DBDictionary();
                    var storeId = namedObjectsDict.SetAt(DwgGlobalStoreName, dwgGlobalStore);
                ...
                trans.Abort();
                return new FlexDataStore(namedObjectsDict.GetAt(DwgGlobalStoreName));
            }
        }
