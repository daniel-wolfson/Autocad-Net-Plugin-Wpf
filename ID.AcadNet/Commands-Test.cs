...
using Intellidesk.AcadNet.Common.GUIs;
using Intellidesk.AcadNet.Common.Internal;
using Intellidesk.AcadNet.Tests;
using System;
using System.Collections.Generic;
using System.Linq;

#if (TEST)
[assembly: CommandClass(typeof(CommandsTest))]
#endif
namespace Intellidesk.AcadNet.Tests
{
    /// <summary>
    /// Tests and samples
    /// </summary>
    public class CommandsTest
    {
        ...
        public static void PolyClean3()
        {
            ...
            ids.XForEach<Polyline>(poly =>
            {
                ...
                var poly1 = DrawObject.Pline(pts);
        ...
        public static void PolySplit()
        {
            ...
            ids.XOpenForWrite<Polyline>(list =>
            {
                foreach (var poly in list)
                {
                    ...
                    foreach (var poly1 in list)
                    {
                        if (poly1 != poly)
                        {
                            ((Entity)poly).IntersectWith3264(poly1, Intersect.OnBothOperands, intersectPoints);
                        }
                    }
            ...
            if (newPolys.Count > 0)
            {
                ...
                ids.XForEach(entity => entity.Erase());
            }
        ...
        public static void PolyTrimExtend() // mod 20130228
        {
            ...
            var ids = Interaction
                .GetSelection("\nSelect polyline", "LWPOLYLINE")
                .XWhere(pline => visibleLayers.Contains(pline.Layer) && pline.Visible)
        ...
        public static void SaveSelection()
        {
            ...
            var handles = ids.XSelect(entity => entity.Handle.ToString()).ToArray();
        ...
        {
            var ids = Interaction.GetSelection("\nSelect MText", "MTEXT");
            var mts = ids.XOpenForRead<MText>().Select(mt =>
            {
                var dt = DrawObject.Text(mt.Text, mt.TextHeight, mt.Location, mt.Rotation, false, mt.TextStyleName);
                dt.Layer = mt.Layer;
                return dt;
            }).ToArray();
            ids.XForEach(mt => mt.Erase());
        ...
        {
            var ids = Interaction.GetSelection("\nSelect Text", "TEXT");
            var dts = ids.XOpenForRead<DBText>().Select(dt =>
            {
                var mt = DrawObject.MText(dt.TextString, dt.Height, dt.Position, dt.Rotation, false);
                mt.Layer = dt.Layer;
                return mt;
            }).ToArray();
            ids.XForEach(dt => dt.Erase());
        ...
        public static void ShowExtents() // newly 20130815
        {
            ...
            var rectId = Drawing.Rectang(extents.MinPoint, extents.MaxPoint);
            Interaction.GetString("\nPress ENTER to exit...");
            rectId.XOpenForWrite(rect => rect.Erase());
        }
        ...
        {
            var ids = QuickSelection.XSelectAll().ToArray();
        ...
        public static void PolyLanding()
        {
            ...
            while (true)
            {
                ...
                var landings = ids.XSelect(entity => (entity as Curve).GetClosestPointTo(p, false)).ToArray();
                ...
                landingLineIds.Add(Drawing.Line(p, landing));
            }
            landingLineIds.XForEach(entity => entity.Erase());
        }
        ...
        public static void PolylineInfo() // mod by WY 20130202
        {
            ...
            var poly = id.XOpenForRead<Polyline>();
            ...
            {
                txtIds.Add(Drawing.MText(i.ToString(), height, poly.GetPointAtParameter(i), 0, true));
            }
            Interaction.GetString("\nPress ENTER to exit");
            txtIds.XForEach(mt => mt.Erase());
        }
        ...
        public static void ShowLayerName()
        {
            ...
            if (result == 0)
            {
                ...
                ids
                    .XWhere(entity => !entity.Layer.Contains("_Label"))
                    .XSelect(entity => entity.Layer)
                    .Distinct()
            ...
            ids.XForEach<Entity>(entity =>
            {
                string layerName = entity.Layer;
                if (!layerName.Contains("_Label"))
                {
                    var center = entity.GetCenter();
                    var text = DrawObject.MText(layerName, height, center, 0, true);
        ...
        public static void InspectObject()
        {
            ...
            Gui.PropertyPalette(id.XOpenForRead());
        }
        ...
        {
            var bId = Drawing.Block(QuickSelection.XSelectAll(), "test");
        ...
        public void TestRegion()
        {
            ...
            Drawing.Boundary(point, BoundaryType.Region);
        }
        ...
        {
            var ids = QuickSelection.SelectAll("LWPOLYLINE").XWhere(pline => pline.GetCode() == "parcel").ToArray();
            ids.XForEach<Polyline>(poly =>
            {
        ...
        {
            var lineId = Drawing.Line(Point3d.Origin, Point3d.Origin + Vector3d.XAxis);
            lineId.SetLayer("aaa");
        }
        ...
        public void TestGroup()
        {
            var ids = Interaction.GetSelection("\nSelect entities");
            ids.Group();
            var groupDict = HostApplicationServices.WorkingDatabase.GroupDictionaryId.XOpenForRead<DBDictionary>();
        ...
        {
            Drawing.Hatch(new[] { new Point3d(0, 0, 0), new Point3d(100, 0, 0), new Point3d(0, 100, 0) });
        }
        ...
        public void TestArc()
        {
            ...
            Drawing.Circle(point1, 5);
            var point2 = Interaction.GetPoint("\nMid");
            Drawing.Circle(point2, 5);
            var point3 = Interaction.GetPoint("\nEnd");
            Drawing.Circle(point3, 5);
            Drawing.Arc3P(point1, point2, point3);
        }
        ...
        public void TestArc2()
        {
            ...
            Drawing.Circle(start, 5);
            var center = Interaction.GetPoint("\nCenter");
            Drawing.Circle(center, 5);
            double angle = Interaction.GetValue("\nAngle");
            Drawing.ArcSCA(start, center, angle);
        }
        ...
        public void TestSpline()
        {
            ...
            while (true)
            {
                ...
                points.Add(point);
                Drawing.Circle(point, 5);
            }
            Drawing.SplineCV(points.ToArray(), true);
        }
        ...
        public void TestPolygon()
        {
            ...
            Drawing.Circle(end, 5);
            Drawing.Polygon(n, center, end);
        }
        ...
        public void TestText()
        {
            ...
            Drawing.Text("FontAbc", 100, Point3d.Origin, 0, true);
        }
        ...
        public void TestTable()
        {
            ...
            Drawing.Table(Point3d.Origin, "Numbers", contents, 5, 20, 2.5);
        }
        ...
        public void TestLayout()
        {
            ...
            var layout = LayoutManager.Current.CreateLayout("TestLayout").XOpenForRead<Layout>();
        ...
        {
            var id = Interaction.GetEntity("\nSelect curve");
            var cv = id.XOpenForRead<Curve>();
            double length = Interaction.GetValue("\nInterval");
            Drawing.Measure(cv, length, new DBPoint());
        }
        ...
        public void TestDivide()
        {
            ...
            Drawing.Divide(cv, num, new DBPoint());
        }
        ...
        {
            var point = Interaction.GetPoint("\nPick one point");
            Drawing.Boundary(point, BoundaryType.Polyline);
        }
        ...
        {
            var ids = Interaction.GetSelection("\nSelect entities");
            var ents = ids.XSelect(entity => entity).ToArray();
            Drawing.Hatch("SOLID", ents);
        }
        ...
        public void TestPolygonMesh()
        {
            ...
            Drawing.PolygonMesh(points, m, n);
        }

        [CommandMethod("TestAddAttribute")]
        public void TestAppendAttribute()
        {
            var id = Drawing.Insert("test", Point3d.Origin);
            ...
            id.XOpenForWrite<BlockReference>(br =>
            {
