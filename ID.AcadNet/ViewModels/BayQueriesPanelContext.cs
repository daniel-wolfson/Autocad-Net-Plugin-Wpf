...
{
    public class BayQueriesPanelContext : BaseViewModel, IBayQueriesPanelContext, IExecutablePanelViewModel
    {
        private IPluginSettings _appSettings;
        public Action<object> Load { get; set; }

        #region <ctor>
        public BayQueriesPanelContext()
        {
            ...
            string[] withAttrTags)
        {
            ...
            var invokeMethod = new Action<CancellationToken>((token) =>
            {
                relevantIds.ForEach(async ent =>
                {
                    if (!IsCanceled)
                    {
                        ObjectIdItemArgs objectIdItem =
                            await ent.Value.XGetDisplayItem<BlockReference, ObjectIdItemArgs>(token, withAttrTags);
        ...
        public override void OnDocumentActivated(object sender, DocumentCollectionEventArgs e)
        {
            ...
            {
                base.OnDocumentToBeDestroyed(sender, e);
                this.ExecuteResetCommand((object)0);
                base.OnDocumentActivated(sender, e);
            });
        }
