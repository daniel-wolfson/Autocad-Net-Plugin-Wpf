...
namespace FileExplorer.Shell
{
    ...
    public class DataSourceShell : IDisposable
    {
        ...
        public DataSourceShell()
        {
            ...
            {
                Plugin.Logger.Error($"{nameof(DataSourceShell)} ShellAPI.SHGetSpecialFolderLocation Failed:{{0}}", ex);
            }
        ...
        public DataSourceShell(ShellAPI.IShellFolder shellDesktop, IntPtr pIDL, DataSourceShell shellParent, bool hasOnlyFolders, SpecialFolderType folderType)
        {
            ...
            {
                Plugin.Logger.Error($"{nameof(DataSourceShell)} shellParent.shellFolder.BindToObject Failed:{{0}}", ex);
            }
        }
        ...
        public DataSourceShell(ShellAPI.IShellItem psiDirectory)
        {
            ...
            if (psiDirectory.GetAttributes(ShellAPI.SFGAO.SFGAO_FILESYSTEM, out sfgAttribs) == 0)
            {
                ...
                {
                    Plugin.Logger.Error($"{nameof(DataSourceShell)} ShellAPI.ILCreateFromPath Failed:{{0}}", ex);
                }
            }
        }
        ...
        static System.Windows.Media.ImageSource GetImageSourceFromIcon(IntPtr hIcon)
        {
            ...
            {
                Plugin.Logger.Error($"{nameof(DataSourceShell)} GetImageSourceFromIcon Failed:{{0}}", ex);
            }
            catch (Exception ex)
            {
                Plugin.Logger.Error($"{nameof(DataSourceShell)} error: {{0}}", ex);
            }
            return null;
        }
        #endregion
        ...
        private static void GetSpecialFolders()
        {
            ...
            pidl = IntPtr.Zero;
            ShellAPI.SHGetSpecialFolderLocation(IntPtr.Zero, ShellAPI.CSIDL.CSIDL_PERSONAL, ref pidl);
            listSpecialFolders.Add(new SPECIAL_FOLDER { FolderType = SpecialFolderType.MyDocuments, PIDL = pidl });

            pidl = IntPtr.Zero;
            ShellAPI.SHGetSpecialFolderLocation(IntPtr.Zero, ShellAPI.CSIDL.CSIDL_INTERNET, ref pidl);
            listSpecialFolders.Add(new SPECIAL_FOLDER { FolderType = SpecialFolderType.Internet, PIDL = pidl });

            pidl = IntPtr.Zero;
            ShellAPI.SHGetSpecialFolderLocation(IntPtr.Zero, ShellAPI.CSIDL.CSIDL_BITBUCKET, ref pidl);
            listSpecialFolders.Add(new SPECIAL_FOLDER { FolderType = SpecialFolderType.RecycleBin, PIDL = pidl });

            pidl = IntPtr.Zero;
            ShellAPI.SHGetSpecialFolderLocation(IntPtr.Zero, ShellAPI.CSIDL.CSIDL_DRIVES, ref pidl);
            listSpecialFolders.Add(new SPECIAL_FOLDER { FolderType = SpecialFolderType.MyComputer, PIDL = pidl });

            pidl = IntPtr.Zero;
            ShellAPI.SHGetSpecialFolderLocation(IntPtr.Zero, ShellAPI.CSIDL.CSIDL_NETWORK, ref pidl);
            listSpecialFolders.Add(new SPECIAL_FOLDER { FolderType = SpecialFolderType.Network, PIDL = pidl });

            pidl = IntPtr.Zero;
            ShellAPI.SHGetSpecialFolderLocation(IntPtr.Zero, ShellAPI.CSIDL.CSIDL_COMMON_DOCUMENTS, ref pidl);
            listSpecialFolders.Add(new SPECIAL_FOLDER { FolderType = SpecialFolderType.SharedDocuments, PIDL = pidl });

            pidl = IntPtr.Zero;
            ShellAPI.SHGetSpecialFolderLocation(IntPtr.Zero, ShellAPI.CSIDL.CSIDL_CONTROLS, ref pidl);
            listSpecialFolders.Add(new SPECIAL_FOLDER { FolderType = SpecialFolderType.MyComputerControlPanel, PIDL = pidl });

            pidl = IntPtr.Zero;
            ShellAPI.SHGetSpecialFolderLocation(IntPtr.Zero, ShellAPI.CSIDL.CSIDL_PROFILE, ref pidl);
            listSpecialFolders.Add(new SPECIAL_FOLDER { FolderType = SpecialFolderType.CurrentUserProfile, PIDL = pidl });

            listSpecialFolders.Add(new SPECIAL_FOLDER { FolderType = SpecialFolderType.ApplicationData, PIDL = new IntPtr(99999) });
        }
        ...
        public List<DataSourceShell> GetFolderByPage(int pageSize, int pageindex, out bool isEndPage)
        {
            ...
            List<DataSourceShell> lst = new List<DataSourceShell>();
            Plugin.Logger.Information($"{nameof(DataSourceShell)} shell GetFolderByPage:{{0}}", this.ParsingName);
            ...
            {
                isEndPage = true;
                Plugin.Logger.Debug($"{nameof(DataSourceShell)} result != 0 ==> true:", this.ParsingName);
                return null;
            }
            #endregion
            ...
            while (itemIndex < pageSize)
            {
                ...
                {
                    Plugin.Logger.Debug("enum-1   : {0}", ptrIDLChild);
                    ...
                    Plugin.Logger.Debug("enum  : {0}", ptrIDLChild);
                }
                // Free the PIDL and reset counters.
                Marshal.FreeCoTaskMem(ptrIDLChild);
                ptrIDLChild = IntPtr.Zero;
                ...
                {
                    Plugin.Logger.Debug("ptrIDLChild.Equals(IntPtr.Zero) && iGotChild == 0   ==> true");
                    isEndPage = true;
                    break;
                }
                itemIndex++;
