...
{
    public class SearchViewModel : SortOrderViewModel, ISearch, ISearchViewModel
    {
        #region <Properties>
            ...
            set
            {
                if (SetProperty(ref searchKeyword, value, "SearchKeyword", "IsSearchEnabled"))
                {
                    if (!this.searchKeyword.IsNullOrEmpty()
                        && Files.IsValidFilename(searchKeyword))
                    {
        ...
        public bool IsSearchEnabled
        {
            ...
            set
            {
                SetProperty(ref _isSearchEnabled, value, "IsSearchEnabled");
            }
        }
        public bool IsMessageInfoVisible
        {
            get
            {
        ...
        public IBasePanelContext Parent { get; set; }
        ...
        #region <ICommand>
        private ICommand _searchCommand { get; }
        private ICommand _searchClearCommand { get; }
        public ICommand SearchCommand => _searchCommand;
        public ICommand SearchClearCommand => _searchClearCommand;
        #endregion

        #region ctor
        public SearchViewModel()
        {
            _searchCommand = new DelegateCommand<object>(ExecuteSearch);
            _searchClearCommand = new DelegateCommand<object>(ExecuteSearchClear);
        ...
        public void ExecuteSearchClear(object value)
        {
            SearchKeyword = string.Empty;
        }

        public void ExecuteSearch(object value)
        {
            ...
            InfraManager.DelayAction(200, async () =>
            {
                ...
                if (commandArgs.CancelToken.IsCancellationRequested) return;

                //NotifyArgs notifyResult = new ReadyNotifyArgs();
                //HttpServiceResult serviceResult = Files.FindPath(SearchKeyword, null);
                //if (serviceResult.ActionCode == HttpStatusCode.Found && Parent != null)
                //    Parent.ExecuteCommand(new CommandArgs(null, "Search", serviceResult.ActionResult));
                //else
                //    notifyResult = new ErrorNotifyArgs(ActionMessage.ToString());
                //InfraManager.RunOnUIThread(() => { Mouse.OverrideCursor = null; Notifications.DisplayNotifyMessage(notifyResult); });
            });
        }
        ...
        {
            this.FolderDetailsCollectionView.Filter = null;
            switch (mediaType)
            {
                case MediaType.Image:
                    RunOnUIThreadAsync(() =>
                    {
                        this.FolderDetailsCollectionView.Filter = (item) =>
                        {
    ...
    public class StringValidationError : ValidationRule
    {
        ...
        {
            if (!Files.IsValidFilename(value.ToString()))
