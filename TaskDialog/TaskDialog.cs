...
namespace TaskDialogInterop
{
	public partial class TaskDialog
	{
		...
		private static TaskDialogResult ShowTaskDialog(TaskDialogOptions options)
		{
			var vtd = new VistaTaskDialog
			    {
			        ...
			        MainInstruction = options.MainInstruction,
			        Content = options.Content,
			        ExpandedInformation = options.ExpandedInfo,
			        Footer = options.FooterText
			    };

		    if (options.CommandButtons != null && options.CommandButtons.Length > 0)
			{
				var lst = new List<VistaTaskDialogButton>();
				for (int i = 0; i < options.CommandButtons.Length; i++)
				{
					try
					{
						var button = new VistaTaskDialogButton
						    {
						        ButtonId = GetButtonIdForCommandButton(i),
						        ButtonText = options.CommandButtons[i]
						    };
					    lst.Add(button);
					}
			...
			{
				var lst = new List<VistaTaskDialogButton>();
				for (var i = 0; i < options.RadioButtons.Length; i++)
				{
					try
					{
						var button = new VistaTaskDialogButton
						    {
						        ButtonId = GetButtonIdForRadioButton(i),
						        ButtonText = options.RadioButtons[i]
						    };
					    lst.Add(button);
					}
					catch (FormatException)
			...
			if (options.CustomButtons != null && options.CustomButtons.Length > 0)
			{
				...
				for (var i = 0; i < options.CustomButtons.Length; i++)
				{
					try
					{
						var button = new VistaTaskDialogButton
						    {
						        ButtonId = GetButtonIdForCustomButton(i),
						        ButtonText = options.CustomButtons[i]
						    };
            ...
            vtd.CanBeMinimized = options.CanBeMinimized;
			...
			vtd.Config = options;
		    vtd.Width = options.Width;

			TaskDialogResult result = null;
			int diagResult = 0;
			var simpResult = TaskDialogSimpleResult.None;
			bool verificationChecked = false;
			int radioButtonResult = -1;
			int? commandButtonResult = null;
			int? customButtonResult = null;

            diagResult = vtd.Show((vtd.CanBeMinimized ? IntPtr.Zero : options.Handle), out verificationChecked, out radioButtonResult);
		...
		private static TaskDialogResult ShowEmulatedTaskDialog(TaskDialogOptions options)
		{
			...
			var tdvm = new TaskDialogViewModel(options);
